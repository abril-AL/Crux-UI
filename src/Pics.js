...
// f = 0-50  m 51-77 (?)
export var images = [f1, f2, f3, f4, f5, f6, f7, f8, f9, f10,
...
function Pics(user) {
    ...
    const prefRange = get_pref_imgs(SO);
    const prefImgs = images.slice(prefRange['l'], prefRange['h'] + 1);
    const [randOrder, setRandOrd] = useState(getRandImgs(SO));
    ...
    if (b) {
        if (i == 10) {
            return (<>
                <PostTrial user={user} imgs={randOrder}></PostTrial>
            </>);
        }
        return (
            <>
        ...
        return (
            <div>
                <Countdown width={368} tgt={TIME} event={() => { seti(i + 1); setb(true); }} ></Countdown>
                <div>
                    <Image height={400} src={images[randOrder[i]]}  ></Image>
                </div>
            </div>
        );
    }
    ...
    if (so != '') {
        if (so === "Heterosexual (female)" || so === "Homosexual (male)") {
            while (list.length < 10) {
                ...
                const randomNumber = Math.floor(Math.random() * (MAX - MID + 1)) + MID;
        ...
        if (so === "Heterosexual (male)" || so === "Homosexual (female)") {
            while (list.length < 10) {
                const randomNumber = Math.floor(Math.random() * (MID + 1));
                if (!list.includes(randomNumber)) {
                    list.push(randomNumber);
                }
            }
